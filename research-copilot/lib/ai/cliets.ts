...
export function tuning(maxTokens: number, temperature: number) {
  ...
  return reasoningEra
    ? {
        // The budget covers reasoning tokens *and* visible output. A limit
        // sized for the answer alone gets eaten by the thinking, and the call
        // returns truncated JSON with finish_reason "length". Triple it, and
        // keep reasoning shallow: these are extraction and summarisation tasks
        // against a supplied evidence pack, not problems that reward deep
        // deliberation.
...
export async function completeJson<S extends z.ZodTypeAny>(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    // A truncated response is not a schema problem, and reporting it as one
    // sends you looking in the wrong place. Say which it was.
    if (choice?.finish_reason === "length") {
      throw new Error(
        `Model response was cut off at the token limit (${maxTokens} requested). ` +
          `On reasoning models the limit covers thinking as well as output — raise maxTokens for this task.`
      );
    }
